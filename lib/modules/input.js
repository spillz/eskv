...
export class InputHandler {
    ...
    ensureWidgetVisible(widget) {
        ...
        /**
         * `getTransformRecurse(true, widget)` returns the transform that maps
         * from the widget's *parent* coordinate space into App coordinates.
         * We then translate by the widget's own offset so that the resulting
         * matrix converts local widget coordinates (origin at 0,0) into the
         * App space that ScrollView expects when handling `focus_visible`.
         */
        let parentTransform = new DOMMatrix();
        const parent = widget.parent;
        if (parent && typeof parent.getTransformRecurse === 'function') {
            parentTransform = parent.getTransformRecurse(true, widget);
        }
        const transform = parentTransform.multiply(
            new DOMMatrix().translate(rect[0], rect[1])
        );
        const corners = [
            [0, 0],
            [rect[2], 0],
        ...
        const points = corners.map(([x, y]) => {
            const pt = transform.transformPoint(new DOMPoint(x, y));
            return [pt.x, pt.y];
        });
        ...
        const appRect = new Rect([minX, minY, maxX - minX, maxY - minY]);
