...
export class InputHandler {
     ...
     * @param {boolean} [includeOffscreenScrollDescendants=true]
     * @returns {boolean}
     */
    focusWidget(widget, includeOffscreenScrollDescendants = true) {
        if (widget && !this.isWidgetFocusable(widget, includeOffscreenScrollDescendants)) {
            widget = null;
        }
        if (widget) {
    ...
    /**
     * @param {Widget|null} widget
     * @param {boolean} [includeOffscreenScrollDescendants=false]
     * @returns {boolean}
     */
    isWidgetFocusable(widget, includeOffscreenScrollDescendants = false) {
        ...
        for (const parent of widget.iterParents()) {
            ...
            if (!includeOffscreenScrollDescendants
                && this.isScrollContainer(parent)
                && !this.isWidgetVisibleInScrollContainer(widget, parent)) return false;
            child = parent;
        }
        return true;
    }
     ...
     * @returns {Widget[]}
     */
    getFocusableWidgets(scope = this.getFocusScopeRoot(), includeOffscreenScrollDescendants = false) {
        this.ensureFocusLayoutCurrent(scope);
        const focusables = [];
        for (let widget of scope.iter(true, false)) {
            if (this.isWidgetFocusable(widget, includeOffscreenScrollDescendants)) focusables.push(widget);
        }
        return focusables;
    }
    ensureFocusValid() {
        ...
        if (this.focusedWidget && !this.isWidgetFocusable(this.focusedWidget, true)) {
     ...
     * @returns {boolean}
     */
    moveFocusWithin(scope, direction, wrap = this.focusWrap, includeOffscreenScrollDescendants = false) {
        if (!this.focusedWidget || !this.isDescendantOf(this.focusedWidget, scope)) return false;
        const focusables = this.getFocusableWidgets(scope, includeOffscreenScrollDescendants).filter(widget => widget !== scope);
        ...
        this.focusWidget(best, includeOffscreenScrollDescendants);
        return true;
    }
    /**
     * @param {'left'|'right'|'up'|'down'} direction
