...
export class VoxelTileMap extends LayeredTileMap {
    ...
    draw(app, ctx) {
            ...
            for (let x = x0; x < x1; x++) { // east west
                for (let y = y0; y < y1; y++) { //north south
                    ...
                    if (voxelType > 0) {
                        ...
                        if (this._aLayer && this._aLayer.get([x,y])===0 && this._vLayer) {
                            ...
                            const viz = this._vLayer.get([x,y])===1;
                            if (bottom !== undefined && front === undefined && z >= elev && viz) {
                                this.spriteSheet.drawIndexed(oCtx, bottom, x, y - z + 1);
                                darkPositions.set((y-z+1)*td0+x, [x, y - z + 1]);
                                }
                            if (back !== undefined && top === undefined && z>0) {
                                this.spriteSheet.drawIndexed(oCtx, back, x, y - z);
                                darkPositions.set((y-z)*td0+x, [x, y - z]);
                                }
                            if (front !== undefined && z>0) {
                            ...
                            if (top !== undefined && z >= elev) {
                                ...
                                if (leftLedge !== undefined && this._maxElevation.get([x - 1, y]) < z) {
                                    this.spriteSheet.drawIndexed(oCtx, leftLedge, x - 1, y - z);
                                    darkPositions.set((y-z)*td0+x-1, [x-1, y - z]);
                                }
                                if (rightLedge !== undefined && this._maxElevation.get([x + 1, y]) < z) {
                                    this.spriteSheet.drawIndexed(oCtx, rightLedge, x + 1, y - z);
                                    darkPositions.set((y-z)*td0+x+1, [x+1, y - z]);
                                }
                                if (backLedge !== undefined && this._maxElevation.get([x, y - 1]) < z) {
                                    this.spriteSheet.drawIndexed(oCtx, backLedge, x, y - z - 1);
                                    darkPositions.set((y-z-1)*td0+x, [x, y - z-1]);
                                }
                            }    
                        } else {
                            let [ top, front, back, bottom, leftLedge, backLedge, rightLedge ] = this.voxelIndex[voxelType] || [undefined];
                            if (bottom !== undefined && front === undefined && z >= elev) {
                                this.spriteSheet.drawIndexed(oCtx, bottom, x, y - z + 1);
                            }
                            if (back !== undefined && top === undefined && z>0) {
                                this.spriteSheet.drawIndexed(oCtx, back, x, y - z);
        ...
        const s = this.spriteSheet.spriteSize;
        ctx.clearRect(this.x+x0, this.y+y0, x1-x0, y1-y0);
