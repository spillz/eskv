...
/** @typedef {{ tileDim: VecLike, _data?: number[], _layerData?: number[][], activeLayer?: number, numLayers?: number, spriteSheet: string|null }} TileMapSerializedData */
/** @typedef {TileMapSerializedData & { _layerData: number[][], activeLayer: number, numLayers: number }} LayeredTileMapSerializedData */
 ...
 * @param {number} len 
 * @param {number} sw 
 * @returns {[boolean, number, number, number, boolean]}
 */
export function unpackSpriteInfo(ind, len, sw) {
	const orientation = Math.floor(ind/len);
	const flipX = (orientation & 4) !== 0;
	const flipY = (orientation & 8) !== 0;
	const angle = orientation & 3;
	ind = ind%len;
	const indY = Math.floor(ind/sw);
	const indX = ind%sw;
	return [flipX, angle, indY, indX, flipY]
}

/**
 * 
 * @param {Vec2} spriteIndex 2D reference
 * @param {boolean} flipX horizontally flipped
 ...
 * @param {boolean} [flipY=false] vertically flipped
 * @returns 
 */
export function packSpriteInfo2D(spriteIndex, flipX, angle, len, sw, flipY=false) {
	return spriteIndex[0] + spriteIndex[1]*sw + angle*len + (flipX?4*len:0) + (flipY?8*len:0);
}
 ...
 * @returns 
 */
export function packSpriteInfo(spriteIndex, flipX, angle, len, flipY=false) {
	return spriteIndex + angle*len + (flipX?4*len:0) + (flipY?8*len:0);
}
...
export class SpriteSheet extends Resource {
    ...
    serialize() {
        ...
        const animations = {}
        for (const [k, animation] of this.animations.entries()) animations[String(k)] = animation;
     ...
     * @param {boolean} flipY
     */
    drawIndexed(ctx, ind, x, y, extraAngle=0, flipX = false, dx=0, dy=0, flipY=false) {
        if(ind<-1) {
            ind = this.animations.get(ind)?.tileValue??-1;
        }
        if(ind>=0) {
            let [packedFlipX, angle, indY, indX, packedFlipY] = unpackSpriteInfo(ind, this.len, this.sw);
            const combinedFlipX = packedFlipX !== flipX;
            const combinedFlipY = packedFlipY !== flipY;
            angle = (angle + extraAngle) % 4;
            if(!combinedFlipX && !combinedFlipY && angle===0) {
                this.draw(ctx, [indX,indY], x+dx, y+dy);
            } else {
                this.drawRotated(ctx, [indX,indY], x+0.5, y+0.5, angle*90, combinedFlipX, [0.5-dx,0.5-dy], combinedFlipY);
            }
        }
    }
    /**
     ...
     * @param {boolean} [flipY=false]
     */
    drawRotated(ctx, spriteLoc, x, y, angle, flipx=false, anchor='center', flipY=false){
        ctx.save();
//        let flipped = 1 - 2*flipx;
     ...
     * @param {[number, number]|'center'} [anchor='center']
     */
...
export class SpriteWidget extends Widget {
    ...
    /**
     * @param {string} e
     * @param {Widget|import('./widgets.js').EventSink} o
     * @param {any} v
     */
    on_frames(e,o,v) {
        this.restart();
	...
	draw(app, ctx) {
        ...
        if(this.frame instanceof LayeredAnimationFrame) {
            for(let [f, dx, dy] of this.frame.iter()) {
                this.spriteSheet.drawIndexed(ctx, f, 0, 0, this.facing, this.flipX, dx, dy, this.flipY);
            }
        } else {
            this.spriteSheet.drawIndexed(ctx, this.frame, 0, 0, this.facing, this.flipX, 0, 0, this.flipY);
        }
...
export class TileMap extends Widget {
    ...
    serialize() {
        ...
        data['spriteSheet'] = Array.from(App.resources.keys()).find((k)=>App.resources.get(k)===this.spriteSheet)??null;
        return data;
    }
    /**
     * 
    ...
    /**
     * 
     * @param {TileMapSerializedData} data 
     */
    deserialize(data) {
        ...
        const spriteSheetKey = data['spriteSheet'];
        this.spriteSheet = spriteSheetKey === null ? null : /** @type {SpriteSheet|null} */ (App.resources.get(spriteSheetKey) ?? null);
        this._data._cacheData = null;
    }
...
export class LayeredTileMap extends TileMap {
     ...
     */
    deserialize(data) {
        const layerData = data['_layerData'] ?? [];
        this.numLayers = data['numLayers'] ?? layerData.length;
    ...
    draw(app, ctx) {
        ...
        for (let z = 0; z < this.numLayers; z++) { //height
            ...
            for (let c of this.children) { //entities get rendered at the level above their reported elevations
                if (z === /** @type {any} */ (c).elevation + 1) c.draw(app, oCtx);
            }
        }
