 ...
 * as is a color tint argument per tile or per vertex. Draw calls are batched in blocks of 100 and flushed
 */
export class WebGLTileRenderer {
    ...
    constructor(canvas) {
        ...
        this.vertexData = new Float32Array(100 * 6 * (2 + 2 + 4)); // 100 quads max, 6 vertices each, (pos + texCoord + color)
...
export class WebGLTileWidget extends Widget {
    constructor(props = {}) {
        super();
        /**@type {{sx:number, sy:number, sw:number; sh:number, dx:number, dy:number, dw:number; dh:number, tint: [number, number, number, number]}[]} */
        this.drawBatch = [];
