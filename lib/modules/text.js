...
/**
 * @param {Array<string|number>} textArray
 */
export function rightPad(textArray) {
    let finalText = "";
    textArray.forEach(text => {
        let txt = String(text);
        for (let i = txt.length; i < 10; i++) {
            txt += " ";
        }
        finalText += txt;
    });
 ...
 */
export function sizeText(ctx, text, size, fontName, centered, rect, color, paddingRatio=1) {
    let scale = 1;
    if (size < minFontSize) {
 ...
 * @returns {[number, number]} width and height needed to render the text at given font size
 */
export function sizeWrappedText(ctx, text, size, fontName, centered, rect, color, wordwrap = true, paddingRatio = 1) {
