...
export class Color extends Array {
    ...
    static fromString(colorName) {
        ...
        if(colorName.startsWith("#")) {
          ...
          const r = parseInt(match[1], 16)??0;
          const g = parseInt(match[2], 16)??0;
          const b = parseInt(match[3], 16)??0;
          const a = 0;
      ...
      if(colorName.startsWith("rgba(")) {
        ...
        const lookedUp = colorLookup[colorName];
        if (!lookedUp) throw Error(`Unkwown color string ${colorName}`);
        let [r,g,b] = lookedUp["rgb"];
        return new Color(r,g,b,1)
    }
    /**
...
/** @type {Record<string, { name: string, rgb: [number, number, number] }>} */
