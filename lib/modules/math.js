...
export class MathArray extends Array {
    ...
    abs() {
        return Math.abs.apply(null, /** @type {any} */ (this));
    }
    /**
     * 
     * @param {number} k 
    ...
    /**
     * @param {(value:number, index:number, array:number[]) => boolean} func
     */
    filter(func) {
...
class Node {
  ...
  class WeightedGraph {
    constructor() {
      /** @type {Record<string, { node: string, weight: number }[]>} */
      this.adjacencyList = {};
    }
    /**
     * @param {string} vertex
      ...
      while (nodes.values.length) {
        ...
        if (smallest === finish) {
          ...
          while (previous[smallest]) {
            path.push(smallest);
            smallest = /** @type {string} */ (previous[smallest]);
          }
          break;
        }
