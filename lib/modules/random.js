...
/**
 * Normalize any numeric seed into an unsigned 32-bit number.
...
export class PRNG_sfc32 extends PRNG {
    ...
    seed(seed) {
        ...
        this._seed = seed32;
        this.xorSeed = (seed32 ^ this.d) >>> 0; // 32-bit seed with optional XOR value
        const sm = splitmix64(this.xorSeed);
        this.random = sfc32(
            Number(sm() & 0xffffffffn) >>> 0,
            Number(sm() & 0xffffffffn) >>> 0,
            Number(sm() & 0xffffffffn) >>> 0,
            Number(sm() & 0xffffffffn) >>> 0
        );
