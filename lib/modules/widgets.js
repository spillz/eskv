...
export class EventSink {
     ...
     * @param {PropertyBag} properties Object containing properties to udpate
...
export class Widget extends Rect {
    ...
    constructor() {
        ...
        /**@type {PropertyBag|null} Internal flag to indicate deferred property initialization */
        this._deferredProps = null;
...
export class Label extends Widget {
    ...
    layoutChildren() {
        ...
        if ('h' in this.hints && this.hints['h'] == null) {
            if (fontSize !== null) {
                this[3] = this.richText? sizeRichText(ctx, this.text, fontSize, this.fontName, this.rect, this.color, this.wrap, this.fontHeightPadding)[1] :  
                    this.wrap ? sizeWrappedText(ctx, this.text, fontSize, this.fontName, this.align == "center", this.rect, this.color, this.wrapAtWord, this.fontHeightPadding)[1] :
                    sizeText(ctx, this.text, fontSize, this.fontName, this.align == "center", this.rect, this.color, this.fontHeightPadding)[1];
            }
        }
        if ('w' in this.hints && this.hints['w'] == null) {
            if (fontSize !== null) {
                this[2] = this.richText? sizeRichText(ctx, this.text, fontSize, this.fontName, this.rect, this.color, this.wrap, this.fontHeightPadding)[0] :  
                    this.wrap ? sizeWrappedText(ctx, this.text, fontSize, this.fontName, this.align == "center", this.rect, this.color, this.wrapAtWord, this.fontHeightPadding)[0] :
                    sizeText(ctx, this.text, fontSize, this.fontName, this.align == "center", this.rect, this.color, this.fontHeightPadding)[0];
            }
        }
...
export class ScrollView extends Widget {
    ...
    /**
     * Returns touch position in App space (tiles), independent of this widget transform.
     * @param {import('./input').Touch} touch
     * @returns {Vec2}
     */
    _getAppTouchPos(touch) {
        const nativeObject = touch.nativeObject;
        if (nativeObject && 'clientX' in nativeObject && 'clientY' in nativeObject) {
            return new Vec2(App.get().toChild([nativeObject.clientX, nativeObject.clientY]));
        }
        return new Vec2([touch.x, touch.y]);
    }
    /**@type {EventCallbackNullable} */
    /**
     * 
    ...
    *iter(recursive = true, inView = true) {
    ...
    on_touch_down(event, object, touch) {
        ...
        if (this.collide(r)) {
            let millis = Date.now();
            let tp = this._getAppTouchPos(touch);
            this._oldTouch = [tp.x, tp.y, touch.identifier, millis, null];
            if (super.on_touch_down(event, object, touch)) return true;
            touch.grab(this);
            return true;
        }
    ...
    on_touch_move(event, object, touch) {
        ...
        let r = touch.rect;
        if (this.collide(r)) {
            let appTouchPos = this._getAppTouchPos(touch);
            //Pan
            if (this.uiMove && (touch.nativeEvent == null || (touch.nativeEvent instanceof TouchEvent && touch.nativeEvent.touches.length == 1))) { // || touch.nativeEvent.touches.length==2
                ...
                if (this._oldTouch != null && touch.identifier == this._oldTouch[2]) {
                    ...
                    let dx = appTouchPos.x - ot[0];
                    let dy = appTouchPos.y - ot[1];
                    if (this.scrollW) {
                        this.scrollX = this.scrollableW == 0 ? 0 : (this._scrollX - dx / this.zoom);
                    }
                    if (this.scrollH) {
                        this.scrollY = this.scrollableH == 0 ? 0 : (this._scrollY - dy / this.zoom);
                    }
                    let time = Date.now()
                    let vel = new Vec2([0, 0]), ovel = new Vec2([0, 0]);
                    if (ot != null) {
                        let millis = Math.max(time - ot[3], 1);
                        let vx = (this.scrollW && (this.scrollableW > 0 || this.unboundedW)) ? (-dx / this.zoom) / millis : 0;
                        let vy = (this.scrollH && (this.scrollableH > 0 || this.unboundedH)) ? (-dy / this.zoom) / millis : 0;
                        vel = new Vec2([vx, vy]);
                        ovel = ot[4] !== null ? ot[4] : ovel;
                    }
                    let nvel = vel.abs().sum() === 0 ? vel : vel.add(ovel).scale(0.5);
                    this._oldTouch = [appTouchPos.x, appTouchPos.y, touch.identifier, time, nvel];
                }
            }
            //Zoom
