...
export class Rules {
     ...
     * @param {Object<string, any>} ruleProperties Object containing the default properties to be applied for this class
...
export class EventSink {
    ...
    **/
    ...
    deferredProperties(app) {
        ...
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function') { //Dynamic property binding
                ...
                let args, rval;
                [args, rval] = /**@type {string}*/(func['text'] ?? func.toString()).split('=>');
                args = args.replace('(', '').replace(')', '').split(',').map(a => a.trim());
                let objs = args.map(a => app.findById(a));
                /**@type {Object<string, Widget|EventSink|null>} */
                let obmap = {}
                for (let a of args) {
     ...
     * @param {Object<string, any>} properties Object containing properties to udpate
     ...
     * @param {(event:string, obj: this, data:any)=>boolean|null|undefined|void} func callback function to trigger when property changes
     */
    listen(event, func) {
...
export class TextInput extends Label {
    ...
    constructor() {
        super();
        /**@type {ReturnType<getWrappedTextData>|ReturnType<getRichText>|ReturnType<getTextData>|null} */
        this._textData = null;
    }
    /**@type {Widget['on_touch_down']} */
    on_touch_down(event, object, touch) {
    ...
    _syncTextareaVAlign(inp, rt, lhPx) {
        ...
        const lines = Math.max(1, (this._textData?.lines?.length) || 1);
