...
/**
 * Layout hints interpreted by a widget's parent layout.
 *
 * Value semantics:
 * - numeric values are proportions of the parent/reference size on that axis
 * - numeric strings such as `'8'` are tile units
 * - suffixed strings such as `'0.5w'`, `'0.5h'`, `'2ww'`, `'2wh'`, `'10aw'`, and `'10ah'`
 *   are measured relative to another dimension or reference frame as interpreted by `getMetric()`
 * - omitting a hint leaves that axis/property parent-driven
 * - `null` for `w` or `h` requests intrinsic/content-driven sizing when the parent layout supports it
 *
 * Layout-specific notes:
 * - BoxLayout treats an omitted main-axis size hint as flexible remaining-space allocation
 * - BoxLayout treats `w:null` or `h:null` as fit-to-children on the relevant axis
 * - GridLayout also supports intrinsic sizing via `null`, but only when its row/column sizing resolves
 *   to fixed dimensions; see GridLayout docs for details
 *
 * @typedef {{
 *   x?: number|string,
 *   y?: number|string,
...
/** @typedef {Record<string, any> & { id?: string }} PropertyBag */
...
export class Rules {
     ...
     * @param {Record<string, any>} ruleProperties Object containing the default properties to be applied for this class
     * @param {boolean} replace If true, removes all current properties (if they exist) and replaces them with `ruleProperties`. If false, merges with current properties.
     */
...
export class EventManager {
    ...
    listen(listener, emitterEvent, callback) {
        ...
        const emitterId = second === undefined ? /** @type {any} */ (listener)["id"] ?? listener : first;
    ...
    emit(emitter, event, data) {
        const conns = this.connections.get(/** @type {any} */ (emitter)["id"] ?? emitter);
    ...
    update(app, millis) { //todo: props can be of the form {prop1: 1, prop2: 2, ...} or {prop1: [1,func1], prop2: [2,func2]}
        ...
        if (this.elapsed == 0) {
            ...
            for (let p in targetProps) {
                this.initProps[p] = /** @type {any} */ (this.widget)[p]; //TODO: If the window gets resized these will be out of date
            }
        }
        ...
        if (skip < 0) {
            for (let p in this.initProps) {
                let x = targetProps[p];
                if (typeof x === 'number') {
                    const start = /** @type {number} */ (this.initProps[p]);
                    /** @type {any} */ (this.widget)[p] = (1 - wgt) * start + wgt * x;
            ...
            if (this.stack.length == 0) this.cancel();
                ...
                for (let p in this.stack[0][0]) {
                    this.initProps[p] = /** @type {any} */ (this.widget)[p];
                }
            }
        }
    }
...
export class EventSink {
    ...
    constructor() {
        ...
        return new Proxy(this, {
            set(target, name, value, receiver) {
                if (typeof name === 'string' && name[0] == '_') return Reflect.set(target, name, value, receiver);
    ...
    **/
    ...
    deferredProperties(app) {
        ...
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function') { //Dynamic property binding
                ...
                /** @type {string} */
                let rval = '';
                const expression = String(func['text'] ?? func.toString());
                const [argsExpr, rvalExpr = ''] = expression.split('=>');
                args = argsExpr.replace('(', '').replace(')', '').split(',').map((a) => a.trim()).filter(a => a.length > 0);
                rval = rvalExpr;
                let objs = args.map(a => app.findById(a));
                /** @type {Record<string, Widget|EventSink|null>} */
                let obmap = {}
                for (let a of args) {
     ...
     * @param {Record<string, any>} properties Object containing properties to udpate
    ...
    emit(event, data, listener) {
        if ('on_' + event in this) {
            if (/** @type {any} */ (this)['on_' + event](event, this, data)) return true;
        }
    ...
    *iterByPropertyValue(property, value) {
        for (let w of this.iter(true, false)) {
            const record = /** @type {Record<string, any>} */ (w);
            if (property in record && record[property] == value) yield w;
        }
    }
    /**
...
export class Widget extends Rect {
    ...
    outlineColor = null;
    /**@type {boolean} Indicates whether this widget currently has focus*/
    focus = false;
    /**@type {boolean} Indicates whether this widget can receive focus*/
    canFocus = false;
    /**@type {string} Color used to draw the focus outline*/
    focusOutlineColor = 'rgba(255, 255, 255, 0.85)';
    /**@type {number|string|null} Optional width used for the focus outline*/
    ...
    /**@type {WidgetSizeHints} Layout constraints interpreted by the parent; see `WidgetSizeHints` for value semantics and layout-specific notes.*/
    hints = {};
    /**
    ...
    constructor() {
        ...
        /**@type {PropertyBag|null} Internal flag to indicate */
        this._deferredProps = null;
    ...
    **/
     ...
     * Hints are active layout constraints interpreted by the parent, not soft preferences.
     * See `WidgetSizeHints` for value semantics and layout-specific behavior.
     *
     ...
     * @param {(...args: (typeof this)[]) => any} computeFn - Function that takes widgets (by ID) and returns computed value.
     */
     ...
     * @param {Partial<this>&{id?:string}} properties Object containing properties to udpate
     */
    updateProperties(properties) {
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function' && !('markupMethod' in properties[p])) { //Dynamic property binding
    ...
    /**
     * Bubble an event up through the widget ancestry until it is handled or the root is reached.
     * @param {string} event
     * @param {any} data
     * @param {boolean} [includeSelf=true]
     * @returns {boolean}
     */
    bubbleEvent(event, data, includeSelf = true) {
    ...
    * @yields {Widget}
    */
    /** @returns {Generator<Widget, void, unknown>} */
    *iter(recursive = true, inView = true) {
        yield this;
        if (!recursive) return;
    ...
    *iterByPropertyValue(property, value) {
    ...
    appToChild(pos, recurse = true) {
        let transform = this.getTransformRecurse(true);
        let pt = transform.inverse().transformPoint(new DOMPoint(pos[0], pos[1]));
        return new Vec2([pt.x, pt.y]);
    }
    /**
    ...
    removeChild(child, disconnect = true) {
        ...
        const inputHandler = App.appInstance?.inputHandler;
        if (inputHandler?.focusedWidget === child) {
            inputHandler.focusWidget(null);
        }
    ...
    set children(children) {
        for (let c of this._children) {
            const inputHandler = App.appInstance?.inputHandler;
            if (inputHandler?.focusedWidget === c) inputHandler.focusWidget(null);
    ...
    /**
     * Handler for generic activation events such as keyboard/gamepad select.
     * Returns false by default so the event can bubble to parents.
     * @param {string} event
     * @param {Widget} object
     * @param {*} data
     * @returns {boolean}
     */
    on_activate(event, object, data) {
        return false;
    }
    /**
     * Handler for escape/cancel events triggered from the focused widget.
     * Returns false by default so the event can bubble to parents.
     * @param {string} event
     * @param {Widget} object
     * @param {*} data
     * @returns {boolean}
     */
    on_escape(event, object, data) {
        return false;
    }
    /**
     * Handler for directional focus input before normal focus traversal.
     * Returns false by default so the input can move focus normally.
     * @param {string} event
     * @param {Widget} object
     * @param {{direction:'left'|'right'|'up'|'down', activateHeld?:boolean}|null} data
     * @returns {boolean}
     */
    on_focus_direction(event, object, data) {
        return false;
    }
    /**
    ...
    draw(app, ctx) {
        ...
        if (this.outlineColor != null || (this.focus && this.canFocus)) {
            const originalLineWidth = ctx.lineWidth;
            const originalStroke = ctx.strokeStyle;
            const baseLineWidth = this.getMetric(this, 'lineWidth', this.hints.lineWidth ?? `${1.0 / app.tileSize}`);
            if (this.outlineColor != null) {
                ctx.lineWidth = baseLineWidth;
                ctx.strokeStyle = this.outlineColor;
                ctx.stroke();
            }
            if (this.focus && this.canFocus) {
                const focusWidth = this.focusOutlineWidth != null
                    ? this.getMetric(this, 'focusOutlineWidth', this.focusOutlineWidth)
                    : Math.max(baseLineWidth, 2 / app.tileSize);
                ctx.lineWidth = focusWidth;
                ctx.strokeStyle = this.focusOutlineColor;
                ctx.stroke();
            }
...
/** @typedef {Record<string, [{ a: (...args:any[]) => Widget } & (new () => Widget), string]>} WidgetClassInfo */
...
export class App extends Widget {
     ...
     * 
     * @param {string} name 
     * @param {{ a: (...args:any[]) => Widget } & (new () => Widget)} cls 
    ...
    addModal(modal) {
        ...
        this._needsLayout = true;
        this.inputHandler?.onFocusScopeChanged();
    }
    /**
    ...
    *iterByPropertyValue(property, value) {
    ...
    setupCanvas() {
        ...
        const cssWidth = rect.width > 0 ? rect.width : this.w;
        const cssHeight = rect.height > 0 ? rect.height : this.h;
        const bufferWidth = Math.max(1, Math.round(cssWidth * dpr));
        const bufferHeight = Math.max(1, Math.round(cssHeight * dpr));
        if (this.canvas.width !== bufferWidth || this.canvas.height !== bufferHeight) {
            this.canvas.width = bufferWidth; //this.tileSize*(this.dimW);
            this.canvas.height = bufferHeight; //this.tileSize*(this.dimH);
        }
...
export class Label extends Widget {
    ...
    color = "white";
    /** @type {ReturnType<typeof getTextData>|ReturnType<typeof getWrappedTextData>|ReturnType<typeof getRichText>|null} */
    _textData = null;
    /**
    ...
    sizeToGroup(ctx) {
        ...
        for (let lbl of /** @type {Iterable<Label>} */ (App.get().iterByPropertyValue('sizeGroup', this.sizeGroup))) {
            const best = lbl._bestFontSize;
            if (best !== undefined && fontSize > best && !lbl.ignoreSizeForGroup) fontSize = best;
        }
        if (fontSize > 0) {
            for (let lbl of /** @type {Iterable<Label>} */ (App.get().iterByPropertyValue('sizeGroup', this.sizeGroup))) {
                const fs = lbl.clip || !lbl.ignoreSizeForGroup ? fontSize : (lbl._bestFontSize ?? fontSize);
                if (lbl.richText) {
...
export class Button extends Label {
    ...
    on_touch_down(event, object, touch) {
        ...
        if (!this.disable && this.collide(touch.rect)) {
            touch.grab(this);
            this._touching = true;
            App.get().inputHandler?.focusWidget(this);
            return true;
        }
        return false;
    }
    ...
    on_touch_up(event, object, touch) {
        ...
        if (this.collide(touch.rect)) {
            if (this.group === null || !this.singleSelect) {
                ...
                if (this.parent !== null) {
                    for (let w of /** @type {Iterable<ToggleButton>} */ (this.parent.iterByPropertyValue('group', this.group))) {
    ...
    on_touch_up(event, object, touch) {
        ...
        if (!this.disable && this.collide(touch.rect)) {
            ...
            if (this.group == null) {
                ...
                for (let w of /** @type {Iterable<CheckBox>} */ (this.parent.iterByPropertyValue('group', this.group))) {
...
export class Slider extends Widget {
    ...
    adjustValue(direction) {
        if (this.disable || direction === 0) return false;
        const max = this.max ?? this.curMax;
        const range = max - this.min;
        if (range <= 0) return false;
        const step = this.step ?? range / 20;
        let value = clamp(this.value + step * direction, this.min, max);
        if (this.step != null) value = Math.round(value / this.step) * this.step;
        this.value = value;
        this.updateMax();
        return true;
    }
    /**@type {Widget['on_focus_direction']} */
    on_focus_direction(event, object, data) {
        const direction = data?.direction;
        if (this.orientation === 'horizontal') {
            if (direction === 'left') return this.adjustValue(-1);
            if (direction === 'right') return this.adjustValue(1);
        } else {
            if (direction === 'up') return this.adjustValue(-1);
            if (direction === 'down') return this.adjustValue(1);
        }
        return false;
    }
    /**@type {Widget['on_touch_down']} */
    ...
    draw(app, ctx) {
        ...
        if (this.focus && this.canFocus) {
            ...
            const focusWidth = this.focusOutlineWidth != null
                ? this.getMetric(this, 'focusOutlineWidth', this.focusOutlineWidth)
                : Math.max(baseLineWidth, 2 / app.tileSize);
            const focusRect = this.rect;
            ctx.beginPath();
            ctx.rect(focusRect[0], focusRect[1], focusRect[2], focusRect[3]);
            ctx.lineWidth = focusWidth;
            ctx.strokeStyle = this.focusOutlineColor;
            ctx.stroke();
            ctx.lineWidth = originalLineWidth;
 ...
 * contains the text in either state. The TextInput exposes an `editing` property, which is
...
export class TextInput extends Label {
    ...
    /** @type {import('./text.js').TextRenderingData|null} */
    _textData = null;
    ...
    addDOMInput() {
        if (!this._textData || this._activeDOMInput) return;
        const app = App.get();
 ...
 *
 * Main-axis sizing rules:
 * - Children with an explicit main-axis hint (`hints.h` in a vertical BoxLayout,
 *   `hints.w` in a horizontal BoxLayout) are treated as fixed/requested-size children first.
 * - Children without a main-axis hint are flexible and receive a share of the remaining space.
 * - A hint like `hints:{h:1}` or `hints:{w:1}` requests the full parent allocation on that axis;
 *   it does not mean "take the leftover space after siblings".
 *
 * Practical guidance:
 * - In a vertical BoxLayout, content widgets such as ScrollView should usually omit `hints.h`
 *   if they are meant to grow beneath a fixed-height header/footer.
 * - In a horizontal BoxLayout, omit `hints.w` for children that should expand into the remaining width.
 */
export class BoxLayout extends Widget {
    ...
    *iterChildren() {
    ...
    /**
     * Lays out children along the main axis.
     * Explicit main-axis hints consume space first; children without a main-axis hint
     * receive the remaining space.
     */
    layoutChildren() {
        if (this._layoutNotify) this.emit('layout', null);
        this._needsLayout = false;
...
export class TabbedNotebook extends Notebook {
    ...
    on_tabHeightHint(e, o, v) {
        if (!this.buttonScroller) return;
        this.buttonScroller.hints['h'] = this.tabHeightHint;
        this._needsLayout = true;
    }
    /**@type {EventCallbackNullable} */
    ...
    updateButtons() {
        ...
        for (let page of this.children) {
            const tb = a(ToggleButton, {
                text: /** @type {any} */ (page)['name'] ?? String(i + 1),
 ...
 *
 * Sizing notes:
 * - GridLayout uses the same `WidgetSizeHints` value semantics as other widgets.
 * - Children with explicit row/column-relevant hints contribute fixed sizes first.
 * - `w:null` or `h:null` requests intrinsic sizing, but GridLayout only fits itself to children
 *   when the corresponding columns/rows resolve to fixed dimensions.
 */
export class GridLayout extends Widget {
    ...
    layoutChildren() {
        ...
        if (this.orientation === 'horizontal') {
            const numX = Math.max(1, this.numX);
            const numY = Math.max(1, Math.ceil(this.children.length / numX));

            // Available space given our current size (may be refined after "fit" below)
            let hAvail = this.h - spacingY * (numY - 1) - 2 * paddingY;
            let wAvail = this.w - spacingX * (numX - 1) - 2 * paddingX;

            // Pass 1: measure fixed sizes (including 'null' -> intrinsic)
            let _colWidths = new Array(numX).fill(0);
            let _rowHeights = new Array(numY).fill(0);

            let r = 0, c = 0, i = 0;
            for (let ch of this.children) {
                // Let hints resolve first (percentages/strings), using current avail box
                this.applyHints(ch, wAvail, hAvail);

                // If the child says "auto" (null), it needs its own layout to expose intrinsic size
                if ('w' in ch.hints && ch.hints['w'] === null) ch.layoutChildren();
                if ('h' in ch.hints && ch.hints['h'] === null) ch.layoutChildren();

                if ('w' in ch.hints) _colWidths[c] = Math.max(ch.w, _colWidths[c]);
                if ('h' in ch.hints) _rowHeights[r] = Math.max(ch.h, _rowHeights[r]);

                if ((i + 1) % numX === 0) { r++; c = 0; } else { c++; }
                i++;
            }

            // Totals and how many fixed cols/rows we have
            let fixedW = 0, fixedH = 0, nfX = 0, nfY = 0;
            for (let cw of _colWidths) { fixedW += cw; if (cw > 0) nfX++; }
            for (let rh of _rowHeights) { fixedH += rh; if (rh > 0) nfY++; }

            // If our own width/height hints are null AND all cols/rows are fixed => fit GridLayout to children
            if (nfX === numX && this.hints && this.hints['w'] === null) {
                const oldW = this[2];
                const newW = fixedW + spacingX * (numX - 1) + 2 * paddingX;
                if (Math.abs(oldW - newW) > 1e-6) {
                    this[2] = newW;
                    if (typeof App !== 'undefined' && App.get) App.get()._needsLayout = true;
                }
            }
            if (nfY === numY && this.hints && this.hints['h'] === null) {
                const oldH = this[3];
                const newH = fixedH + spacingY * (numY - 1) + 2 * paddingY;
                if (Math.abs(oldH - newH) > 1e-6) {
                    this[3] = newH;
                    if (typeof App !== 'undefined' && App.get) App.get()._needsLayout = true;
                }
            }

            // Recompute avail (we might have been resized by a parent on a later pass)
            hAvail = this.h - spacingY * (numY - 1) - 2 * paddingY;
            wAvail = this.w - spacingX * (numX - 1) - 2 * paddingX;

            // Flexible shares for any non-fixed columns/rows
            let ch = numY > nfY ? (hAvail - fixedH) / (numY - nfY) : 0;
            let cw = numX > nfX ? (wAvail - fixedW) / (numX - nfX) : 0;

            // If everything is fixed but container has explicit size, center like BoxLayout
            if (this.hints && this.hints['h'] !== null && nfY === numY && hAvail > fixedH) {
                paddingY = (hAvail - fixedH) / 2;
            }
            if (this.hints && this.hints['w'] !== null && nfX === numX && wAvail > fixedW) {
                paddingX = (wAvail - fixedW) / 2;
            }

            // Pass 2: place and size
            let y = this.y + paddingY;
            let x = this.x + paddingX;
            r = 0; c = 0;
            for (let k = 0; k < this.children.length; k++) {
                const el = this.children[k];
                const cw0 = _colWidths[c] === 0 ? cw : _colWidths[c];
                const ch0 = _rowHeights[r] === 0 ? ch : _rowHeights[r];
        ...
        // === Vertical orientation (transpose logic) ===
        const numY = Math.max(1, this.numY);
        const numX = Math.max(1, Math.ceil(this.children.length / numY));
...
export class ScrollView extends Widget {
    ...
    unboundedW = false;
    /**@type {number} Fraction of the viewport moved for keyboard/gamepad scroll steps */
    focusScrollStep = 0.5;
    /**@type {Vec2|null} */
    _zoomCenter = null;
    /**
    ...
    constructor() {
        ...
        /**@type {[number, number, number, number, Vec2|null]|null} */
        this._oldTouch = null;
        this._lastDist = null;
    }
    /**
    ...
    /**@type {EventCallbackNullable} */
    on_focus_visible(event, object, data) {
        const target = data?.target;
        const appRect = data?.appRect;
        if (!(target instanceof Widget) || !appRect) return false;
        if (target === this) return false;
        let isDescendant = false;
        for (const parent of target.iterParents()) {
            if (parent === this) {
                isDescendant = true;
                break;
            }
        ...
        const corners = [
            [appRect[0], appRect[1]],
            [appRect[0] + appRect[2], appRect[1]],
            [appRect[0] + appRect[2], appRect[1] + appRect[3]],
            [appRect[0], appRect[1] + appRect[3]],
        ];
        const childPoints = corners.map(([x, y]) => this.appToChild([x, y]));
        const xs = childPoints.map(([x]) => x);
        const ys = childPoints.map(([, y]) => y);
        const minX = Math.min(...xs);
        const maxX = Math.max(...xs);
        const minY = Math.min(...ys);
        const maxY = Math.max(...ys);
        const viewportW = this.w / this.zoom;
        const viewportH = this.h / this.zoom;
        let nextScrollX = this.scrollX;
        let nextScrollY = this.scrollY;
        if ((this.scrollW || this.unboundedW) && viewportW > 0) {
            const left = minX;
            const right = maxX;
            const viewLeft = this.scrollX;
            const viewRight = this.scrollX + viewportW;
            if (left < viewLeft) nextScrollX = left;
            if (right > viewRight) nextScrollX = Math.max(nextScrollX, right - viewportW);
        }
        if ((this.scrollH || this.unboundedH) && viewportH > 0) {
            const top = minY;
            const bottom = maxY;
            const viewTop = this.scrollY;
            const viewBottom = this.scrollY + viewportH;
            if (top < viewTop) nextScrollY = top;
            if (bottom > viewBottom) nextScrollY = Math.max(nextScrollY, bottom - viewportH);
        }
        if (nextScrollX !== this.scrollX) this.scrollX = nextScrollX;
        if (nextScrollY !== this.scrollY) this.scrollY = nextScrollY;
        return false;
    }
    /**@type {Widget['on_activate']} */
    on_activate(event, object, data) {
        const inputHandler = App.get().inputHandler;
        if (!inputHandler || inputHandler.focusedWidget !== this) return false;
        const visibleChild = this.findFirstVisibleFocusableChild();
        if (visibleChild) {
            inputHandler.focusWidget(visibleChild);
            return true;
        }
        return false;
    }
    ...
    on_escape(event, object, data) {
        const inputHandler = App.get().inputHandler;
        if (!this.canFocus || !inputHandler?.focusedWidget || inputHandler.focusedWidget === this) return false;
        if (!inputHandler.isDescendantOf(inputHandler.focusedWidget, this)) return false;
        inputHandler.focusWidget(this);
        return true;
    }
    /**@type {Widget['on_back_button']} */
    on_back_button(event, object, history) {
        return this.on_escape('escape', this, history);
    }
    /**@type {Widget['on_focus_direction']} */
    on_focus_direction(event, object, data) {
        const direction = data?.direction;
        ...
        const inputHandler = App.get().inputHandler;
        if (!inputHandler?.focusedWidget) return false;
        if (inputHandler.focusedWidget === this) {
            if (data?.activateHeld) {
                this.scrollFocusDirection(direction);
                return true;
            }
            return false;
        }
        if (!inputHandler.isDescendantOf(inputHandler.focusedWidget, this)) return false;
        if (inputHandler.moveFocusWithin(this, direction, false, true)) return true;
        return false;
    }
    /**
     * @returns {Widget|null}
     */
    findFirstVisibleFocusableChild() {
        const inputHandler = App.get().inputHandler;
        if (!inputHandler) return null;
        const viewportRect = inputHandler.getWidgetAppRect(this);
        let fallback = null;
        for (const widget of inputHandler.getFocusableWidgets(this)) {
            if (widget === this) continue;
            if (!fallback) fallback = widget;
            if (inputHandler.getWidgetAppRect(widget).collide(viewportRect)) return widget;
        }
        return fallback;
    }
    /**
     ...
     */
    scrollFocusDirection(direction) {
        const stepX = this.w / this.zoom * this.focusScrollStep;
        const stepY = this.h / this.zoom * this.focusScrollStep;
        switch (direction) {
            case 'left':
                if (this.scrollW || this.unboundedW) this.scrollX -= stepX;
                break;
            case 'right':
                if (this.scrollW || this.unboundedW) this.scrollX += stepX;
                break;
            case 'up':
                if (this.scrollH || this.unboundedH) this.scrollY -= stepY;
                break;
            case 'down':
                if (this.scrollH || this.unboundedH) this.scrollY += stepY;
                break;
