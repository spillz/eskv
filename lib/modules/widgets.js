...
export class Rules {
     ...
     * @param {Object} replace If true, removes all current properties and replaces them with `ruleProperties`. If false, merges with current properties.
     */
...
export class Label extends Widget {
    ...
    layoutChildren() {
        ...
        if ('w' in this.hints && this.hints['w'] == null) {
            if (fontSize !== null) {
                this[2] = this.wrap ? sizeWrappedText(ctx, this.text, fontSize, this.fontName, this.align == "center", this.rect, this.color, this.wrapAtWord)[0] :
...
export class ToggleButton extends Label {
    ...
    on_touch_down(event, object, touch) {
        ...
        if (!this.disable && this.collide(touch.rect)) {
            touch.grab(this);
            this._touching = true;
            console.log('touch',this.text, this.parent, this);
            return true;
        }
        return false;
    }
    ...
    on_touch_up(event, object, touch) {
        ...
        touch.ungrab();
        this._touching = false;
        console.log('release',this.text, this.parent, this);
