...
export class Widget extends Rect {
    ...
    */
    ...
    /** 
     * Called internally by the widget to emit property changes to own object on_<event> handlers
     * or to object that bind to the property 
     * @param {string} event name of event (usually the property name) 
     * @param {any} data data value to send
     */
    emit(event, data) {
        if ('on_' + event in this) {
    ...
    *iterByPropertyValue(property, value) {
     ...
     * @type {(recurse?:boolean)=>DOMMatrix} 
     */
    getTransformRecurse() {
        let transform = this.parent ? this.parent.getTransformRecurse() : new DOMMatrix();
    ...
    /**
     * Converts position data from App space to child space
    ...
    /**
     * The framework will call layoutChildren on this widget if the `_needsLayout` flag is set
     ...
     * @param {CanvasRenderingContext2D} ctx The drawing context (relevant transforms will have been applied)
     */
    draw(app, ctx) {
    ...
    update(app, millis) {
        if (this._deferredProps != null) this.deferredProperties();
        if (this._animation != null) this._animation.update(app, millis);
    ...
    *iter(recursive = true, inView = true) {
    ...
    update(app, millis) {
        ...
        if (this.vel !== null) {
            ...
            let a = this.vel.abs();
            if (a.x <= this.velCutoff * this.scrollableW / this.zoom && 
                a.y <= this.velCutoff * this.scrollableW / this.zoom) this.vel = null;
        }
    }
    /**@type {()=>DOMMatrix} */
    getTransform() {
        let transform = new DOMMatrix()
            .translate(this.x - this._scrollX * this.scrollableW * this.zoom,
    ...
    on_touch_move(event, object, touch) {
        ...
        if (this.collide(r)) {
            ...
            //Zoom
            if (this.uiZoom && (touch.nativeEvent instanceof TouchEvent) && touch.nativeEvent.touches.length == 2) {
                ...
                if (this._lastDist != null) {
                    ...
                    let scrollPosCenterW = this._scrollX * this.scrollableW + this.w / this.zoom / 2;
                    let scrollPosCenterH = this._scrollY * this.scrollableH + this.h / this.zoom / 2;
                    ...
                    this.scrollX = (this._zoomCenter.x - this.w / this.zoom / 2) / this.scrollableW;
                    this.scrollY = (this._zoomCenter.y - this.h / this.zoom / 2) / this.scrollableH;
                }
                this._lastDist = d;
            }
            if (super.on_touch_move(event, object, touch)) return true;
    ...
    on_wheel(event, object, touch) {
        ...
        if (this.uiZoom && app.inputHandler.isKeyDown("Control")) {
            ...
            let lx = loc.pos[0]; //*this.scrollableW;
            let ly = loc.pos[1]; //*this.scrollableH;

            let zoom = this.zoom / (1 + 10 * wheel.deltaY / app.h);
            ...
            let mx = moc.pos[0]; //*this.scrollableW;
            let my = moc.pos[1]; //*this.scrollableH;
            ...
            this.scrollX = (this.scrollableW == 0 && !this.unboundedW) ? 0 : (lx - this.w / this.zoom / 2) / this.scrollableW;
            this.scrollY = (this.scrollableH == 0 && !this.unboundedH) ? 0 : (ly - this.h / this.zoom / 2) / this.scrollableH;
            ...
            this.scrollX += (this.scrollableW == 0 && !this.unboundedW) ? 0 : this.w / this.zoom * (wheel.deltaY / app.w) / this.scrollableW;
            if (this.scrollX != this._scrollX) this.scrollX = this._scrollX;
            return true;
        } else if (this.scrollH) {
            this.scrollY += (this.scrollableH == 0 && !this.unboundedH) ? 0 : this.h / this.zoom * (wheel.deltaY / app.h) / this.scrollableH;
            if (this.scrollY != this._scrollY) this.scrollY = this._scrollY;
            return true;
        }
        return false;
