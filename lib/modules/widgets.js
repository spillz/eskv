...
export class EventSink {
    ...
    constructor(properties=null) {
        ...
        return new Proxy(this, {
            set(target, name, value) {
                if(name[0]=='_') return Reflect.set(target, name, value);
     ...
     * @param {EventCallback} func callback function to trigger when property changes
     */
    bind(event, func) {
     ...
     * @param {EventCallback} func callback to unbind
     */
    unbind(event, func) {
...
export class Widget extends Rect {
    ...
    constructor(properties=null) {
        ...
        return new Proxy(this, {
            set(target, name, value) {
                if(['x','y','w','h','children','rect'].includes[name] || name[0]=='_') return Reflect.set(target, name, value);
                Reflect.set(target, name, value);
                if(typeof name==='string') {
                    target.emit(name, value);
                }
    ...
    *iterByPropertyValue(property, value) {
    ...
    /**
     * Converts position data to the coordinates system of the parent widget
    ...
    /**
     * Handler for wheel events activated on the widget
     * @param {string} event 
     * @param {Touch} wheel 
     * @returns 
     */
    ...
    /**
     * Handler for touch or mouse down events activated on the widget
     * @param {string} event 
     * @param {Touch} touch
     * @returns 
     */
    ...
    /**
     * Handler for touch or mouse up events activated on the widget
     * @param {string} event 
     * @param {Touch} touch
     * @returns 
     */
    ...
    /**
     * Handler for touch or mouse move events activated on the widget
     * @param {string} event 
     * @param {Touch} touch
     * @returns 
     */
    ...
    /**
     * Handler for touch or mouse cancel events activated on the widget
     * @param {string} event 
     * @param {Touch} touch
     * @returns 
     */
    ...
    /**
     * Handler for back button (DO NOT USE, DOES NOT WORK)
     * @param {string} event 
     * @param {*} history
     * @returns 
     */
    ...
    /**
     * Set the sizing information for a widget positional property based on hint rules
     * @param {Widget} widget 
     * @param {string} target 
     * @param {string|number} rule 
     * @param {number|null} parentWidth 
    ...
    /**
     * Given the geometry of this widget and the App, sets the geometry of children using their hints property
     * @param {Widget} c 
     * @param {number|null} w 
     * @param {number|null} h 
     */
    ...
    /**
     * The framework will call layoutChildren on this widgets if the _needsLayout flag set
     * and will recursively reposition all child widgets based on positioning
     * data this widget and the hints information of the child widgets.
     * May occasionally be useful to call manually but avoid if possible.
     */
    ...
    /**
     * Draw the widget and its children. This calls the actual draw method of this widget.
     * The purpose of _draw is to allow the drawing context to be udpated by some widgets between 
     * drawing of the current widget and it's children.
     ...
     * (the children's _draw and draw methods are called in this widget's _draw method)
     */
    draw() {
...
export class Label extends Widget {
    ...
    /**
     * Called by the framework if _needsLayout is true to udpate the layout state of the widget's children
     * which in this case is the text data
     */
    layoutChildren() {
        let app = App.get();
        if(!app.ctx) return;
        ...
        if('h' in this.hints && this.hints['h']==null) {
            if(fontSize!==null) {
                ...
                sizeText(app.ctx, this.text, fontSize, this.fontName, this.align=="center", this.rect, this.color)[1]; 
            }
        }
    ...
    /**
     * Called by the framework to render the label text, which was pre-prepared in the layoutChildren method
     */
    draw() {
        super.draw();
        let r = this.rect;
        let ctx = App.get().ctx;
        if(!ctx || !this._textData) return;
        if(this.wrap) {
...
export class Button extends Label {
    ...
    on_touch_down(event, touch) {
        ...
        return false; // super.on_touch_down(event, touch);
    }
    on_touch_up(event, touch) {
        ...
        return false; //super.on_touch_up(event, touch);
    }
	on_touch_move(event, touch) {
		if(touch.grabbed==this && !this.disable) {
			this._touching = this.collide(touch.rect);
		}
		return false; // super.on_touch_move(event, touch);
	}
    draw() {
...
export class CheckBox extends Widget {
    ...
    on_touch_down(event, touch) {
        ...
        return false; //super.on_touch_down();
    }
    on_touch_up(event, touch) {
        ...
        return false; //super.on_touch_up();
    }
	on_touch_move(event, touch) {
		if(touch.grabbed==this && !this.disable) {
			this._touching = this.collide(touch.rect);
...
export class Slider extends Widget {
    ...
    min = 0.0;
    /**@type {number} Max value of slider */
    max = 1.0;
...
/**
 * A TextInput is a Label that when focused is overlaid with an HTML Input element 
...
export class TextInput extends Label {
     ...
     * because changing the window state is assume to make the input lose focus
     * @param {string} event 
     * @param {*} value 
     */
    ...
    addDOMInput() {
        ...
        fs = this._textData.size*app.tileSize;
        if(type=='textarea') {
            let rows = this._textData.outText.length;
            // inp.rows = rows;
        }
        inp.value = this.text;
        //     // fs = (this.fontSize==null?r.h/(rows+1):this._textData.size*app.tileSize);
        // } else {
        //     fs = (this.fontSize==null?r.h/2:this._textData.size*app.tileSize);
        // }
...
export class ImageWidget extends Widget {
     ...
     * @param {ImageWidgetProperties} props 
     */
    constructor(props) {
        super();
        this.updateProperties(props);
    ...
    *iter(recursive=true, inView=true) {
    ...
    update(millis) {
        ...
        if(this.vel !== null) {
            ...
            let a=this.vel.abs();
            if(a.x<=this.velCutoff*this.scrollableW/this.zoom && a.y<=this.velCutoff*this.scrollableW/this.zoom) this.vel=null;
        }
    }
    on_touch_move(event, touch) {
        let r = touch.rect;
        ...
        if(this.collide(r)) {
            ...
            if(touch.nativeEvent==null || touch.nativeEvent.touches.length==1) { // || touch.nativeEvent.touches.length==2
                ...
                if(this._oldTouch!=null && touch.identifier==this._oldTouch[2]) {
                    if(this.scrollW) {
                        this.scrollX = this.scrollableW==0? 0 : (this._scrollX*this.scrollableW + (this._oldTouch[0] - tl.x))/this.scrollableW;
                    }
                    if(this.scrollH) {
                        this.scrollY = this.scrollableH==0? 0 : (this._scrollY*this.scrollableH + (this._oldTouch[1] - tl.y))/this.scrollableH;
                    }
                    ...
                    this._oldTouch = [tln.x, tln.y, touch.identifier, time, (vel.add(ovel)).scale(0.5)];
                }
            } 
            //Zoom
            if(this.uiZoom && touch.nativeEvent!=null && touch.nativeEvent.touches.length==2) {
                ...
                if(this._lastDist != null) {
                    ...
                    let pos0l = [...this.iterParents()].reverse().reduce((prev,cur)=>cur.toLocal(prev), pos0);
                    let pos1l = [...this.iterParents()].reverse().reduce((prev,cur)=>cur.toLocal(prev), pos1);
                    let posctr = [(pos0l[0]+pos1l[0])/2, (pos0l[1]+pos1l[1])/2];
                    let loc = this.toLocal(posctr);
    ...
    on_wheel(event, touch) {
        ...
        if(!this.collide(touch.rect)) return false;
