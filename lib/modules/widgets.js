        ...
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function') { //Dynamic property binding
                ...
                for (let pat of rval.matchAll(re)) {
                    let pr, ob;
                    [ob, pr] = pat.slice(1);
                    if(ob!=='this') obmap[ob].bind(pr, (evt, obj, data) => { 
        ...
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function') { //Dynamic property binding
                ...
                for (let pat of rval.matchAll(re)) {
                    let pr, ob;
                    [ob, pr] = pat.slice(1);
                    if(ob!=='this') obmap[ob].bind(pr, (evt, obj, data) => { try { this[p] = func(...objs) } catch (error) { console.log('Dynamic binding error', this, p, error) } });
                }
                //Immediately evaluate the function on the property
                try {
                    this[p] = func(...objs);
