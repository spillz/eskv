...
import {colorString} from './math.js';
...
//                target[name] = value;
    ...
    *iterByPropertyValue(property, value) {
    ...
    applyHints(c) {
        ...
        if('w_from_h' in hints && hints['w_from_h']!=null) c.w = hints['w_from_h']*this.h;
        if('h_from_w' in hints && hints['h_from_w']!=null) c.h = hints['h_from_w']*this.w;
...
export class Label extends Widget {
    ...
    draw() {
        ...
        let ctx = App.get().ctx;
        if(this.wrap) {
            drawWrappedText(ctx, this._textData);
        } else {
            drawText(ctx, this._textData);
        }
    }
}
...
export class CheckBox extends Widget {
    ...
    // on_parent(e,v) {
    //     if(this.group!=null) {
    //         if([...this.parent.iterByPropertyValue('group',this.group)].length==1) {
    //             this.check = true;
    //         }
    //     }
    // }
...
export class BoxLayout extends Widget {
    ...
    applyHints(c,w=null,h=null) {
        ...
        if('w_from_h' in hints && hints['w_from_h']!=null) c.w = hints['w_from_h']*h;
        if('h_from_w' in hints && hints['h_from_w']!=null) c.h = hints['h_from_w']*w;
