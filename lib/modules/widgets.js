...
export class Label extends Widget {
    ...
    sizeToGroup(ctx) {
        ...
        let fontSize = this._bestFontSize;
        for(let lbl of App.get().iterByPropertyValue('sizeGroup', this.sizeGroup)) {
            if(fontSize>lbl._bestFontSize) fontSize = lbl._bestFontSize;
        }
        if(fontSize>0) {
            for(let lbl of App.get().iterByPropertyValue('sizeGroup', this.sizeGroup)) {
                if (lbl.wrap) {
                    lbl._textData = getWrappedTextData(ctx, lbl.text, fontSize, lbl.fontName, lbl.align, lbl.valign, lbl.rect, lbl.color, lbl.wrapAtWord);
                } else {
                    lbl._textData = getTextData(ctx, lbl.text, fontSize, lbl.fontName, lbl.align, lbl.valign, lbl.rect, lbl.color);
                }
            }
        }
    }
