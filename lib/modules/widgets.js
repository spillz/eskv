    ...
    *iter(recursive=true, inView=true) {
    ...
    on_scrollX(event, value) {
        ...
        this._scrollX = this.children[0].w*this.zoom<this.w ? 
                        align : Math.min(Math.max(0, value),1); //this.children[0].w-this.w/this.zoom
    }
    on_scrollY(event, value) {
        ...
        this._scrollY = this.children[0].h*this.zoom<this.h ? 
                        align : Math.min(Math.max(0, value),1); //this.children[0].h-this.h/this.zoom
    }
    to_local(pos) {
    ...
    update(millis) {
        ...
        if(this.vel !== null) {
            ...
            if(this.vel.abs().sum()<=this.momentumCutoff) this.vel=null;
        }
    }
    on_touch_move(event, touch) {
        let r = touch.rect;
            ...
            if(touch.nativeEvent==null || touch.nativeEvent.touches.length==1) { // || touch.nativeEvent.touches.length==2
                ...
                if(this.oldTouch!=null && tl.identifier==this.oldTouch[2]) {
                    ...
                    if(ot!=null) {
                        ...
                        let vx = this.scrollableW>0? (tln.x-tl.x)/millis/this.scrollableW : 0;
                        let vy = this.scrollableH>0? (tln.y-tl.y)/millis/this.scrollableH : 0;
                        vel = new Vec2([vx, vy]);
