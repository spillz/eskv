...
export class EventSink {
    ...
    bind(event, func, listener=null) {
        App.get().eventManager.bind(listener, this, event, func);
    }
    /** 
    ...
    emit(event, data) {
        ...
        return App.get().eventManager.emit(this, event, data);
...
export class Widget extends Rect {
    ...
    *iterByPropertyValue(property, value) {
    ...
    removeChild(child, disconnect=true) {
        ...
        if(disconnect) App.get().eventManager.disconnect(child);
...
export class TextInput extends Label {
    ...
    clearDOM() {
        if (this._activeDOMInput != null) {
            this.text = this.sanitizeInput(this._activeDOMInput.value);
     ...
     * You can override `sanitizeInput` to change what is populated into the `Label`.
     * @param {string} text 
     * @returns 
     */
    sanitizeInput(text) {
