...
export class EventSink {
        ...
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function') { //Dynamic property binding
                ...
                for (let pat of rval.matchAll(re)) {
                    let pr, ob;
                    [ob, pr] = pat.slice(1);
                    if(ob in obmap) obmap[ob].bind(pr, (evt, obj, data) => { 
    ...
    updateProperties(properties) {
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function' && 'dynamicPropertyAssignment' in properties[p]) { //Dynamic property binding
...
export class Widget extends Rect {
    ...
    updateProperties(properties) {
        ...
        for (let p in properties) {
            if (!p.startsWith('on_') && !p.startsWith('_') && typeof properties[p] == 'function' && 'dynamicPropertyAssignment' in properties) { //Dynamic property binding
                this._deferredProps = properties;
            } else {
