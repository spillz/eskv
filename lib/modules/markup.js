...
function evaluatedProperty(key, value, context, root) {
    if(value instanceof String) {
        ...
        if (key.startsWith('on_')) { //event callback
            return new Function("parent", "root", `return function(event, object, value) {${value}}`).bind(context)(context.parent, root);
        }
        if (value.trim().startsWith('(') && value.indexOf('=>')<value.indexOf('\n')) { //function/method definition
            const func = new Function("parent", "root", `return ${value}`).bind(context)(context.parent, root);
