...
function evaluatedProperty(key, valueStr, context, root) {
    ...
    if (valueStr.trim().startsWith('(') && valueStr.indexOf('=>')<key.indexOf('\n')) { //function/method definition
        return new Function("parent", "root", `return ${valueStr}`).bind(context)(context.parent, root);
    }
    const objs = new Set();
    let objCount = 0;
    ...
    return new Function("parent", "root", functionBody).bind(context)(context.parent, root);
}
...
export function parse(markup) {
    ...
    while(lineNum<lines.length) {
        ...
        if (line.endsWith(':')) {
            ...
            if (indentLevel===0 && line.startsWith('<') && line.includes('@')) {
