...
import {Widget, App, EventSink} from './widgets.js';
 ...
 * @param {Record<string, any>} currentObject the object being parsed
...
function inheritanceDataChain(classes, clsName, usedCls=new Set()) {
    ...
    let chain;
    const classInfo = classes[clsName];
    if (!classInfo) return [];
    const [cls, parentCls] = classInfo;
 ...
 * @param {Widget} widget 
 * @param {Record<string, any>} objectData 
 * @param {Widget} rootWidget 
 */
export function instanceClassData(widget, objectData, rootWidget) {
    //TODO: Currently this handles instancing classes from string-based properties
    // and properties specified in code. We should split that logic up.
    /** @type {Record<string, any>} */
    const props = {};
    ...
    for(let p in merged) {
        ...
        if('children'===p && val instanceof Array) {
            const ch = [];
            for(let c of val) {
                if(c instanceof Object && 'cls' in c) { //TODO: Check that this is correct
                    const [cls, clsExtends] = App.classes[/** @type {any} */ (c)['cls']];
            ...
            try {
                ...
                props[p] = val;
                // throw Error(`Error evaluating property ${p} on widget ${typeof widget} and rootWidget ${typeof rootWidget} with value ${typeof val}\n ${val}`)
            }
        }
    }        
 ...
 * @param {{ a: (...args:any[]) => Widget } & (new (...args:any[]) => Widget)} baseClass 
 * @returns {{ a: (...args:any[]) => Widget } & (new () => Widget)}
 */
function createNamedClass(className, baseClass) {
    return /** @type {{ a: (...args:any[]) => Widget } & (new () => Widget)} */ ({
    ...
    while(lineNum<lines.length) {
        ...
        if (line.endsWith(':')) {
            ...
            if (line.startsWith('<') && line.includes('@')) {
                ...
                const cls = createNamedClass(customClass, App.classes[baseClass][0]); //class extends App.classes[baseClass][0] {};
                const clsData = {};
                ...
                for(let rc of ruledClasses) {
                    ...
                    [lineNum, indentLevel] = parseClassData(lines, rc, lineNum+1, indentLevel, ruleset);
