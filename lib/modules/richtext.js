...
function wrapRich(ctx, atoms, baseSize, fontName, maxWidth) {
    /**@type {{text:string, fg:string|null, bg:string|null, a:number, sizePx:number, width:number}[][]} */
 ...
 * @param {CanvasRenderingContext2D|OffscreenCanvasRenderingContext2D} ctx Canvas 2D context
 ...
 */
export function sizeRichText(ctx, text, size, fontName, rect, color, wordwrap = true, paddingRatio = 1) {
    let scale = 1;
    if (size < minFontSize) {
    ...
    totalH += (lines.length ? size * paddingRatio : size * (1 + paddingRatio)); // empty text: at least one line
